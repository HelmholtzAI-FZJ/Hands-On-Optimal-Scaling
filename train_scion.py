...
from rope_model import GPTConfig, GPT
...
model_args = dict(
    ...
    vocab_size=None,
    dropout=dropout,
    no_parameters_rmsnorm=no_parameters_rmsnorm,
...
# we use Scion'optimizer to re-init the model's parameters
...
for n, p in model.named_parameters():
    if n in ("transformer.wte.weight", "transformer.wpe.weight"):
        ...
        scale = math.sqrt(tensor.shape[1])
        with torch.no_grad():
            scale = scale / (
                torch.sqrt(tensor.pow(2).sum(axis=1, keepdim=True)) + 1e-12
            )
            tensor.mul_(scale)
    elif p.ndim == 2:
        tensor = p.data
...
# for tie weights, no need to init the same weights again
tensor = model.lm_head.weight.data
scale = 1 / math.sqrt(tensor.shape[1])
with torch.no_grad():
    scale = scale / (torch.sqrt(tensor.pow(2).sum(axis=1, keepdim=True)) + 1e-12)
    tensor.mul_(scale)
...
for n, p in model.named_parameters():
    if n in ("transformer.wte.weight"):
        emb_params.append(p)
    elif "lm_head" in n:
        lm_head_paramters.append(p)
    elif p.ndim == 2:
        hidden_params.append(p)


print(
    f"emb_params: {len(emb_params)} | hidden_params: {len(hidden_params)} | lm_head_paramters: {len(lm_head_paramters)}"
)


optim_groups = [
    {
        "params": emb_params,  # embedding + position embedding
        "is_light": False,
    ...
    },
    {
        ...
        "backend": "newtonschulz5",  # 'SVD'
    },
    # we have TIE weights, so we don't need to optimize the lm_head
    {
        "params": lm_head_paramters,
        "is_light": False,
        "is_unconstrained": True,
        "lr": learning_rate,
        "momentum": momentum,
    ...
    if mup_enable_coord_check_logging:
        ...
        for module_name, module in model.named_modules():
            if module_name == "transformer.wte":
                ...
                coord_check_handles.append(
                    module.register_forward_hook(partial(hook, key="last_layer"))
                )
    else:
        coord_check_dict = None
